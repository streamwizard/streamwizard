-- Fix account deletion failing with:
--   update or delete on table "vods" violates foreign key constraint
--   "clips_video_id_fkey" on table "clips"
--
-- clips.video_id -> vods.video_id has no ON DELETE action, so vods must be
-- deleted AFTER the clips that reference them. The previous ordering deleted
-- vods first, tripping the FK. Swap so clips are removed before vods.
