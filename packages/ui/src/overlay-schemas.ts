...
export const overlayItemSchema = z.discriminatedUnion("type", [
  ...
  z.object({ id: z.string().uuid().optional(), scene_id: z.string().uuid(), type: z.literal("irl_speed_widget"), x: z.number().min(0), y: z.number().min(0), w: z.number().min(50), h: z.number().min(50), design_w: z.number().min(1).max(20000), design_h: z.number().min(1).max(20000), crop_top: z.number().min(0).max(20000), crop_right: z.number().min(0).max(20000), crop_bottom: z.number().min(0).max(20000), crop_left: z.number().min(0).max(20000), z_index: z.number().int(), rotation: z.number().min(-360).max(360), opacity: z.number().min(0).max(1), is_visible: z.boolean(), is_locked: z.boolean(), label: z.string().min(1).max(100), config: irlFieldWidgetConfigSchema }),
