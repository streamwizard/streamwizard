...
export type CancelComboboxInputCause = "arrowLeft" | "arrowRight" | "backspace" | "blur" | "deselect" | "escape" | "manual";



export const withTriggerCombobox = <V extends Value = Value, E extends PlateEditor<V> = PlateEditor<V>>(
  editor: E,
  { options: { createComboboxInput, trigger, triggerPreviousCharPattern, triggerQuery }, type }: WithPlatePlugin<TriggerComboboxPlugin, V, E>
) => {
  ...
  editor.insertText = (text) => {
    if (!editor.selection || !matchesTrigger(text) || (triggerQuery && !triggerQuery(editor as PlateEditor))) {
    ...
    if (matchesPreviousCharPattern) {
      const inputNode: TElement = createComboboxInput ? createComboboxInput(text) : { children: [{ text: "" }], type };
