    ...
    const initPlayer = useCallback(() => {
        ...
        if (onPlaybackBlocked) {
            player.addEventListener(
                TwitchPlayerEvents.PLAYBACK_BLOCKED,
                onPlaybackBlocked
            );
        }
...
// Hook to control the player externally
export function useTwitchPlayer() {
    const playerRef = useRef<TwitchPlayer | null>(null);

    const setPlayer = useCallback((player: TwitchPlayer) => {
        playerRef.current = player;
    }, []);
    ...
    const controls = {
        ...
        seek: (timestamp: number) => playerRef.current?.seek(timestamp),
        setChannel: (channel: string) => playerRef.current?.setChannel(channel),
        setVideo: (videoId: string, timestamp?: number) =>
            playerRef.current?.setVideo(videoId, timestamp),
        setCollection: (collectionId: string, videoId?: string) =>
            playerRef.current?.setCollection(collectionId, videoId),
        setQuality: (quality: string) => playerRef.current?.setQuality(quality),
        getQualities: () => playerRef.current?.getQualities(),
        getQuality: () => playerRef.current?.getQuality(),
        getCurrentTime: () => playerRef.current?.getCurrentTime(),
        getDuration: () => playerRef.current?.getDuration(),
        getChannel: () => playerRef.current?.getChannel(),
        getVideo: () => playerRef.current?.getVideo(),
        isPaused: () => playerRef.current?.isPaused(),
        getEnded: () => playerRef.current?.getEnded(),
        getPlaybackStats: () => playerRef.current?.getPlaybackStats(),
        enableCaptions: () => playerRef.current?.enableCaptions(),
        disableCaptions: () => playerRef.current?.disableCaptions(),
    };

    return { setPlayer, controls, player: playerRef.current };
}
