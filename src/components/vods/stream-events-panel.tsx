            ...
            return (
              ...
              >
                <div className="flex items-start gap-3">
                  {/* Event icon */}
                  <div
                    className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full ${info.color} text-white`}
                  >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {displayData.userName && (
                      <p className="text-sm text-muted-foreground truncate">{displayData.userName}</p>
                    )}
                    {displayData.message && (
                      <p className="text-xs text-muted-foreground truncate mt-0.5">{displayData.message}</p>
                    )}
                    {displayData.amount && (
                      <p className="text-xs font-medium text-green-500 mt-0.5">{displayData.amount}</p>
                    )}
                  </div>
                </div>
              </button>
