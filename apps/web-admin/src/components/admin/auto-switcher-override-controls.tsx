...
import { clearSceneOverrideForUser, setSceneOverrideForUser } from "@/actions/auto-switcher";
...
export function AutoSwitcherOverrideControls({
  ...
  /** Owner of the config being controlled — the instance's user, not the admin. */
  userId: string;
  scenes: Scene[];
  ...
  async function hold() {
    ...
    if (!scene) return;
    setBusy(true);
    const result = await setSceneOverrideForUser(userId, scene.sceneUuid, scene.sceneName, duration === "manual" ? null : Number(duration));
    setBusy(false);
  ...
  async function release() {
    setBusy(true);
    const result = await clearSceneOverrideForUser(userId);
    setBusy(false);
