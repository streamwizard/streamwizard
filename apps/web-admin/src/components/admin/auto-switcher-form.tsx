...
import { upsertAutoSwitcherConfigForUser, type AutoSwitcherConfigRow } from "@/actions/auto-switcher";
...
interface AutoSwitcherFormProps {
  /** Owner of the config being edited — the instance's user, not the admin. */
  userId: string;
...
export function AutoSwitcherForm({ userId, initialConfig, scenes, sceneItems, obsConnected }: AutoSwitcherFormProps) {
  ...
  function onSubmit(values: AutoSwitcherFormValues) {
    ...
    toast.promise(
      upsertAutoSwitcherConfigForUser(userId, synced).then((result) => {
