...
| **Poll** | A 20-second poll on a loop: `channel.poll.begin`, a `channel.poll.progress` every second as votes come in, `channel.poll.end` with a winner, then a pause and a new question |
