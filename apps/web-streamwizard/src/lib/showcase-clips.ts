...
async function fetchShowcaseClips(): Promise<RealClipCard[]> {
  try {
    ...
    const live = rows
      .filter((row) => isRenderableThumbnail(row.thumbnail_url))
      .slice(0, 8)
      .map((row) => ({
    ...
    // local stack), so live clips come first and the snapshot pads to eight.
    if (live.length >= 8) return live;
    const seen = new Set(live.map((clip) => clip.id));
    return [...live, ...fallbackClipCards.filter((clip) => !seen.has(clip.id))].slice(0, 8);
  } catch {
    return fallbackClipCards;
  }
}

export const getShowcaseClips = unstable_cache(fetchShowcaseClips, ["showcase-clips", "v3"], {
  revalidate: 3600,
});
