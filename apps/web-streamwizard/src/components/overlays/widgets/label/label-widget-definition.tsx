...
import { LABEL_WIDGET_DEFAULT_SIZE, createDefaultLabelWidgetConfig } from "@repo/ui/overlay";
...
export function createLabelWidgetRootItems(ctx: CreateRootItemContext): OverlayItem[] {
  const id = ctx.nextId();
  const { w, h } = LABEL_WIDGET_DEFAULT_SIZE;
  ...
  return [
    {
      ...
      is_locked: false,
      label: `Label ${n}`,
      config: createDefaultLabelWidgetConfig(),
    },
  ];
}
