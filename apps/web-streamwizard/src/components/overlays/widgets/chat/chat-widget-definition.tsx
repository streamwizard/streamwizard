...
import { CHAT_WIDGET_DEFAULT_SIZE, createDefaultChatWidgetConfig } from "@repo/ui/overlay";
import type { CreateRootItemContext } from "../../registry/overlay-widget-registry.types";

export { CHAT_WIDGET_DEFAULT_SIZE } from "@repo/ui/overlay";

export function createChatWidgetRootItems(
  ctx: CreateRootItemContext
): OverlayItem[] {
  const id = ctx.nextId();
  const { w, h } = CHAT_WIDGET_DEFAULT_SIZE;
  const n =
    ctx.scene.items.filter((i) => i.type === "chat_widget").length + 1;
  return [
    {
      id,
      ...
      is_locked: false,
      label: `Chat ${n}`,
      config: createDefaultChatWidgetConfig(),
    },
  ];
}
