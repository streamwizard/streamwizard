...
import { clearSceneOverride, setSceneOverride } from "@/actions/supabase/auto-switcher";
...
export function AutoSwitcherOverrideControls({
  ...
  async function hold() {
    ...
    if (!scene) return;
    setBusy(true);
    const result = await setSceneOverride(scene.sceneUuid, scene.sceneName, duration === "manual" ? null : Number(duration));
    setBusy(false);
