...
export async function createClipFromVOD({ vodId, vod_offset, duration, title }: CreateClipFromVODProps): Promise<ActionResponse<CreateClipResult>> {
  try {
    ...
    // insert the clip in the pending clips table in supabase
    const { data: clipData, error: clipError } = await supabase.from("pending_clips").insert([
      {
        id: crypto.randomUUID(),
        clip_id: response.data.data[0].id,
        broadcaster_id: broadcasterId,
        status: "pending",
        retry_count: 0,
        max_retries: 20,
    ...
    if (clipError) {
      return {
        success: false,
        error: "clip created but not added to pending clips table",
      };
    }
